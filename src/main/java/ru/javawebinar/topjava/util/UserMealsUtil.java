...
//        getFilteredWithExceeded(mealList, LocalTime.of(7, 0), LocalTime.of(12,0), 2000);
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, Integer> mealByDate = mealList.stream()
                .collect(Collectors.groupingBy(UserMeal::toLocalDate , Collectors.summingInt(UserMeal::getCalories)));

        return mealList.stream().filter(meal -> TimeUtil.isBetween(meal.toLocalTime(), startTime, endTime))
                .map(meal -> createUserMealWithExceed(meal, mealByDate.get(meal.toLocalDate()) > caloriesPerDay))
